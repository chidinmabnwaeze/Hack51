    ...
    <div className="bg-white p-8 rounded-xl shadow-md w-full mt-10 md:w-3/4 mx-auto">
      ...
      {pricing.map((item, index) => (
        <div className="flex items-center justify-between" key={index}>
          <label htmlFor="role" className="block mt-4 mb-2 font-semibold">
            {item.title}
          </label>
      ...
      <section className="bg-white rounded-xl mt-12 ">
        ...
        <div className="bg-[#FF0046] rounded-lg mt-4 p-4 w-full md:w-1/2 text-white">
          <span className="block mt-4 mb-2 px-4 text-2xl font-bold">{`₦${pricing.reduce((acc, item) => acc + parseInt(item.value.replace("₦", "").replace(",", "")), 0).toLocaleString()}`}</span>
        </div>
      </section>
    </div>
  );
